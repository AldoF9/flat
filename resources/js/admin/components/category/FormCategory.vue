    ...
    <v-dialog v-model="dialog" width="1200" style="border-radius: 2px">
        ...
        <v-card :outline="true" rounded="xl" style="padding: 1.5rem">
            <v-card-text>
                    ...
                    <div class="d-flex justify-content-center text-center">
                        ...
                        <div>
                            <h3 class="fields-modal">
                                {{ isUpdate ? "Modificar" : "Agregar" }} Categoria
                            </h3>
                        </div>
                    </div>
                    <hr class="widthHr" />
                ...
                "
                            >
                                {{ isUpdate ? "Guardar" : "+ Agregar Categoria" }}
                            </v-btn>
                        </v-col>
                    </v-form>
                </div>
    ...
    data() {
        return {
            ...
            rules: {
                ...
                descriptionRules: [(v) => !!v || "Phone is required"],
            },
        };
    },
        ...
        async submit() {
            ...
            if (valid) {
                ...
                let route_submit = !this.isUpdate
                    ? route("category.upsert", { idCategory: "FAKE_ID" })
                    : route("category.upsert", { idCategory: this.categoryId });
                axios
